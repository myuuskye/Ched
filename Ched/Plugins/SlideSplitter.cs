    ...
    public class SlideSplitter : IScorePlugin
    {
        ...
        public void Run(IScorePluginArgs args, bool bych, int cch)
        {
            ...
            foreach (var slide in targets.ToList())
            {
                ...
                .ToDictionary(p => p.ParentNote as Slide.StepTap, p => p);
                var first = new Slide() { StartTick = slide.StartTick, Channel = slide.Channel };
                ...
                {
                    var step = new Slide.StepTap(first) { TickOffset = p.TickOffset, IsVisible = p.IsVisible, Channel = p.Channel };
                ...
                var second = new Slide() { StartTick = range.StartTick, Channel = slide.Channel };
                ...
                {
                    var step = new Slide.StepTap(second) { TickOffset = p.TickOffset - offset, IsVisible = p.IsVisible, Channel = p.Channel };
